...
Did not end up using but, for future reference to improve:
...
drawCisgender = function() {
    ...
    text("and most people whose sex assigned at birth is male, grow up to identify as male.", L_wTextOffset, B_hTextOffset - 100, 400, 300);
...
drawGenderQueerWave = function() {
    textSize(25);
    fill(darkgreyCol);
    text("Sometimes, a person's gender identity, or the way they feel or see themselves doesn't fall neatly into the binary of definitions our society has of female or male.", L_wTextOffset, T_hTextOffset - 200, 384, 300);
    text("People who feel this way may describe their gender identity as", L_wTextOffset, 805, 426, 200);
    fill(blueCol);
    text("genderqueer, gender fluid, or gender non-binary", L_wTextOffset, 878, 426, 200);
    text("People who identity as gender queer or gender non-binary may express a combination of masculinity and feminiity, or neither in their gender expression", R_wTextOffset - 100, T_hTextOffset - 200, 517, 200);
    //sine wave
    theta += 0.02;
    let x = theta;
...
function mouseClicked() {
    ...
    } else if (drawFunction === drawTransgender) {
        k = k + 1;
        if (k === 6) {
            k = 0;
        }
